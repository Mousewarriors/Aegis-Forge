...
class DockerOrchestrator:
    ...
    def create_vulnerable_agent_container(
        ...
        try:
            ...
            container = self.client.containers.run(
                ...
                labels={"aegis-forge": "true", "type": "sandbox"}
            )
    ...
    def purge_stale_resources(self):
        """Removes all containers and volumes labeled with aegis-forge=true."""
        try:
            # Cleanup containers
            containers = self.client.containers.list(all=True, filters={"label": "aegis-forge=true"})
            for c in containers:
                try:
                    c.stop(timeout=1)
                    c.remove(force=True)
                    print(f" [DOCKER] Purged stale container: {c.name}")
                except:
                    pass
            # Cleanup volumes
            volumes = self.client.volumes.list(filters={"label": "aegis-forge=true"})
            for v in volumes:
                try:
                    v.remove(force=True)
                    print(f" [DOCKER] Purged stale volume: {v.name}")
                except:
                    pass
            return True
        except Exception as e:
