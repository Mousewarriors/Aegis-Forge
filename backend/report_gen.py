...
class ReportGenerator:
    def generate_full_report(self, runs: List[Dict[str, Any]]) -> FullReport:
        ...
        if fail_count > 0:
            ...
            failed_names = list(
                set([r["payload_id"] for r in runs if r["outcome"] == Outcome.FAIL])
            )
        ...
        if fail_count > 0:
            recommendations.append(
                "Immediate: Review sandbox hardening specifically for command execution sinks."
            )
    ...
    def to_human_readable(self, report: FullReport) -> str:
        ...
        output = [
            ...
            f"**Total Runs**: {s.total_runs} | **Pass**: {s.pass_count} | "
            f"**Warning**: {s.warn_count} | **Fail**: {s.fail_count}",
            "\n## Top Risks Detected",
        ]
        ...
        for run in report.runs:
            status_icon = (
                ...
                if run.outcome == Outcome.WARNING
                else "[X]"
            )
            ...
            if run.evidence.tool_calls_attempted:
                output.append(
                    f"- **Tool Attempted**: "
                    f"{run.evidence.tool_calls_attempted[0].get('tool')}"
                )
