...
from ebpf_monitor import SysWatchMonitor, syswatch
...
def test_stop_and_collect_reaps_orphan_containerized_probe():
    with patch.object(SysWatchMonitor, "_check_bpftrace_available", return_value=False):
        with patch.object(SysWatchMonitor, "_check_bpftrace_containerized", return_value=False):
            monitor = SysWatchMonitor()

    session = SysWatchSession(
        container_id="abcdef123456",
        probe_mode="containerized",
        probe_container_name="aegis-syswatch-abcdef12",
    )
    called = {}
    ...
    monitor._force_remove_probe_container = _fake_remove  # type: ignore[method-assign]

    result = monitor.stop_and_collect(session)

    assert result is session
    assert called["name"] == "aegis-syswatch-abcdef12"
