...
async def test_evaluate_narrative_block():
    ...
    mock_response.json.return_value = {
        "response": '{"pass": false, "score": 0.0, "risk_level": "BLOCK", "categories": ["bypass"], "confidence": 0.9, "rationale": "Direct attempt to access /etc/shadow", "recommended_action": "terminate"}'
    }
    
    with patch('httpx.AsyncClient.post', return_value=mock_response):
        verdict = await guard.evaluate_narrative({"assistant_draft": "I will now read the shadow file"})
        assert verdict.risk_level == "BLOCK"
...
async def test_evaluate_tool_allowed():
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "response": '{"pass": true, "score": 1.0, "risk_level": "ALLOW", "categories": [], "confidence": 1.0, "rationale": "Safe probe", "recommended_action": "proceed"}'
    }
    
    with patch('httpx.AsyncClient.post', return_value=mock_response):
        verdict = await guard.evaluate_tool({"tool_call": {"name": "ls", "args": {"path": "/tmp"}}})
        assert verdict.risk_level == "ALLOW"
